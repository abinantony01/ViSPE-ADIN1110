...
char outputString[NUM_MSGS][MAX_MSG_SIZE] = {
    ...
    "User can define their own behavior for when data is received by defining their own rxCallback",
    "The counterpart to this example demonstrates how to access received data if using callback is not desired",
    "This example uses Serial.println in the callback, this is may not be best practice since it can happen in an interrupt",
    "Basic functionality of sending and recieving data is provided by the ViSPE class",
    "Messages are copied to memory internal to the created object",
    "If more performance is required, or you would like more control over memory, try the Vi_SPE_Advanced class"
};
...
static void rxCallback(byte * data, int dataLen, byte * senderMac)
{
    ...
    if(!adin1110.identicalMacs(senderMac, destinationMAC))
    {
...
void setup() 
{
    ...
    Serial.println("Single Pair Ethernet - Example 1a Basic Send and Receive");
    /* Start up adin1110 */
    ...
    /* Set up callback, to control what we do when data is received */
